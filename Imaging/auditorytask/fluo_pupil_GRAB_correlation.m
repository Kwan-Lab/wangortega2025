...
for ii = 1:nFiles
    ...
    if length(fn_pup) == 1 & length(fn_dff) == 1
        ...
        for tt=1:length(cells.dFF) 
            [c{tt},lags] = xcorr(pupilInt(~isnan(pupilInt)),cells.dFF{tt}(~isnan(pupilInt))-mean(cells.dFF{tt}(~isnan(pupilInt))),'normalized');
            newlag = lags*dt;
            % calculate autocorrelation as well
            [autoCorrFluo{tt},lags] = xcorr(cells.dFF{tt}-mean(cells.dFF{tt}),'normalized');
            newlagFluo = lags*dt;
            
           
            %stem(newlag{tt},c{tt});
        end
        [autoCorrPup,lags] = xcorr(pupilZ,'normalized');
        dtPup = mean(diff(pupil.t));
        newlagPup = lags*dtPup;
        savefilename = fullfile(savematpath,'xcorr-pupil-fluo.mat');
        save(savefilename,'c','newlag','autoCorrFluo','newlagFluo','autoCorrPup','newlagPup');
    end
end