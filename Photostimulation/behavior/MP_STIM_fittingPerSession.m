function MP_STIM_fittingPerSession(dataIndex,save_path)
% %MP_fittingPerAnimal %
    ...
    if (runFit)
        for j = 1:size(dataIndex,1)
            ...
            elseif strcmp(model{k}.name, 'FQ_RPE_CK')
                ...
                % save stats_sim
                save(fullfile(save_path,[session{j},'_FQRPECK.mat']),'stats_sim');
            end
            session{j} = dataIndex.LogFileName{j};
        end
        
        %save behavioral .mat file
        save(fullfile(save_path, [model{k}.fun(4:end) '_perSession.mat']),...
            'session','fitpar','bic');
