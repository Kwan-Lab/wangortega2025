...
if trialData.cutPoint ~= 0
    ...
    for tt = 1:length(fn2)
        if length(trialData.(fn2{tt})) > 1
            if strcmp(fn2{tt}, 'stimulation') == 0 %catch for stimulation b/c it has less trials than other variables
...
%    plot_session_beh_vert(trialData,trials,[],tlabel,time_range);
    ...
    plot_lickrate_byTrialType(lick_trType, 0);
...
%             'entro',...
%             'stats');
savename = strcat(LogFileName(1:end-4), '_beh_cut.mat');
 save(fullfile(savematpath,savename),...
            'trialData','sessionData','logfileData','trials',...
            'lregRCUC_output','lregCRInt_output',...
            'lick_trType','iti_trType','respTime_trType',...
            'entro',...
            'stats','nlike','fitpar','bic');

    %catch for bad sessions
    %if entro < 2.5 || size(trialData.cueTimes,1) < 100 || sum(trials.right)/(sum(trials.left)+sum(trials.right)) < .3 || sum(trials.right)/(sum(trials.left)+sum(trials.right)) > .7
        temp = {LogFileName, entro, size(trialData.cueTimes,1), sum(trials.right)/(sum(trials.left)+sum(trials.right))};
        addition = cell2table(temp, 'VariableNames',["logFileName","Entropy","nTrials","right/total"]);
        lowEntropyTable = [addition;lowEntropyTable];
    %end

    %lowEntropyTable = lowEntropyTable;
    close all;
    clearvars -except i dirs expData lowEntropyTable;
