...
trialData.presCodeSet = 88;   %which is the event code set used?
...
% How many trials?
if trialData.presCodeSet == 1 || trialData.presCodeSet == 88
...
%     end
% end
%% Additional inputs for stimulation
    % necessary only for photostimulation sessions 
    if trialData.presCodeSet == 88
    % Stimulation - which was stimulated
    stimCode = [62;63]; %stimulation code
    trialData.stimulation =  CODE(strcmp(TYPE,'Nothing') & ismember(CODE,stimCode));
    
    % Stimulation - region
    
    stimRegionCodes = [1000:1:1040]; %stimulation code
    trialData.stimulationRegion =  CODE(strcmp(TYPE,'Nothing') & ismember(CODE,stimRegionCodes));
    % regionID was written before ruleCode, current code take the first trial
    % from ruleCode. It is missing the first regionID - I have not changed the
    % ruleCode as this is special to the stimulation data only. Fixed manually.
    % Add first trial ( always control, and delete the last trials regionID)
    trialData.stimulationRegion =  [1000; trialData.stimulationRegion(1:end-1)];
    end 
end
